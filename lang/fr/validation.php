...
| `messages()` du composant concerné, et un test l'exige (voir le paquet
| falcon/booking, `EveryRuleHasAMessageTest`). Ce fichier ne remplace pas ce
| travail : il rattrape ce qui lui échappe.
|
| Deux cas, et deux seulement :
|
| 1. Ce que le moteur refuse en amont de nos formulaires. L'envoi de fichier de
|    Livewire est validé par `FileUploadController::validateAndStore()`, dans
|    une requête HTTP séparée et sans messages : aucun `messages()` de composant
...
| ne se résout et la clé brute s'affiche telle quelle.
|
*/
...
return [
    ...
    'mimes' => 'Le fichier doit être d’un des types suivants : :values.',
    'mimetypes' => 'Le fichier doit être d’un des types suivants : :values.',
    'min' => [
    ...
    |--------------------------------------------------------------------------
    |
    | Volontairement vide : un message sur mesure appartient au `messages()` du
    | composant qui déclare la règle, où il se lit à côté d'elle. Le remplir ici
    | disperserait la même information en deux endroits.
    |
    */
